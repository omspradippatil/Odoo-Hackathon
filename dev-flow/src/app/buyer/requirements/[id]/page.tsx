...
export default function RequirementSuccessPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <WorkspaceLayout role={UserRole.BUYER}>
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4">
        ...
        <div className="text-[10px] font-bold text-navy/40 uppercase tracking-widest mb-2">Requirement Created</div>
        ...
        <Link href={`/buyer/requirements/${resolvedParams.id}/vendors`} className="inline-flex items-center gap-2 px-8 py-4 rounded-2xl bg-navy text-white font-bold hover:bg-navy/90 transition-all shadow-lg shadow-navy/20 active:scale-95">
          Discover Vendors <ArrowRight className="w-5 h-5" />
        </Link>
      </div>
    </WorkspaceLayout>
  );
