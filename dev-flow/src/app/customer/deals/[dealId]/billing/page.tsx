      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-4 md:p-8 lg:py-12">
        ...
        <div className="bg-white rounded-3xl border border-navy/10 shadow-sm p-4 md:p-6 mb-8 flex flex-col md:flex-row gap-4 divide-y md:divide-y-0 md:divide-x divide-navy/5">
          <div className="flex-1 px-4 pt-4 md:pt-0">
            ...
            <div className="text-2xl font-bold text-navy">{localOrder ? formatCurrency(localOrder.total) : '₹9,91,200'}</div>
          </div>
          <div className="flex-1 px-4 pt-4 md:pt-0">
            <div className="text-[10px] font-bold text-lime-700 uppercase tracking-widest mb-1 flex items-center gap-1"><Check className="w-3 h-3" /> Paid {localOrder ? 'Full' : 'Advance'}</div>
            <div className="text-2xl font-bold text-lime-700">{localOrder ? formatCurrency(localOrder.total) : '₹2,52,000'}</div>
          </div>
          {!localOrder && (
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <div className="font-bold text-navy">{localOrder ? `INV-${resolvedParams.dealId}` : 'INV-2048-01'}</div>
                        <div className="text-[9px] font-bold text-lime-700 bg-lime/20 border border-lime/30 px-2 py-0.5 rounded uppercase tracking-widest">{localOrder ? 'Paid' : 'Partially Paid'}</div>
                      </div>
                      <div className="text-xs font-medium text-navy/60 mb-1">Issued: 09 Sep {localOrder ? '' : '• Due: 16 Sep'}</div>
                      <div className="text-sm font-bold text-navy">{localOrder ? formatCurrency(localOrder.total) : '₹9,91,200'} <span className="text-xs font-medium text-navy/50 font-normal ml-1">(incl. GST)</span></div>
                    </div>
                  </div>
                ...
                <div className="space-y-0 relative ml-2">
                  ...
                  <div className="flex gap-4 py-3 relative z-10">
                    ...
                    <div className="pt-0.5">
                      ...
                      <div className="text-sm font-bold text-navy">{localOrder ? 'Full payment reconciled' : 'Advance payment reconciled'}</div>
                      <div className="text-xs font-medium text-navy/60 mt-0.5">{localOrder ? formatCurrency(localOrder.total) : '₹2,52,000'} applied to {localOrder ? `INV-${resolvedParams.dealId}` : 'INV-2048-01'}</div>
                    </div>
                  </div>
